...
## IMPORTANT: Auto-Routing

Before following ANY steps below, first classify what the user wants:

1. **Natural language question** (e.g. "how does X work?", "what is Y?", "explain Z", "why does A connect to B?", any sentence with a question mark or asking about the codebase):
   → Skip the full pipeline. Jump directly to the **"/graphify query"** section below and run a BFS query on `graphify-out/graph.json`. If no graph exists, tell the user to run `/graphify <path>` first.

2. **`query "..."` or `path "..." "..."` or `explain "..."`**:
   → Jump directly to the matching subcommand section below.

3. **A file/folder path** (e.g. `.`, `src/`, `microservices/hotel-service`):
   → Run the full pipeline (Steps 1-9).

4. **`add <url>`**:
   → Jump to the `/graphify add` section.

5. **`--update` or `--cluster-only` or `--watch`**:
   → Jump to the matching section.

Only run the full pipeline (Steps 1-9) when the user provides a path to process. Do NOT run the full pipeline when the user asks a question.
